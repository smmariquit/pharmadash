...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.adjustment_type) {
      return NextResponse.json(
        { success: false, message: 'Adjustment type is required' },
        { status: 400 }
      );
    }
    ...
    if (body.quantity_change === 0) {
      return NextResponse.json(
        { success: false, message: 'Quantity change cannot be zero' },
        { status: 400 }
      );
    }
    ...
    if (!body.adjusted_by) {
      return NextResponse.json(
        { success: false, message: 'Adjusted by user is required' },
        { status: 400 }
      );
    }
    ...
    if (Math.abs(body.quantity_change) > adjustmentThreshold && !body.approval_code) {
      return NextResponse.json(
        { success: false, message: 'Approval code required for large adjustments' },
        { status: 400 }
      );
    }
