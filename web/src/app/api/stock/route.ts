// web/src/app/api/stock/route.ts
...
import { Batch, AuditLog } from '@/models';

// Stock Operations Service
class StockOperationsService {
  private store_id = "store_123"; // In real implementation, get from auth context
  ...
  private createStockMovement(
    ...
    return {
      ...
      reference_document_type: reference_document_type as any,
      reason,
      unit_cost,
      timestamp,
      created_by,
  ...
  private validateStockOperation(operation: any, type: string): StockValidationResult {
    const errors = [];
    ...
    switch (type) {
      case 'receive':
        if (!operation.po_id) errors.push({ field: 'po_id', message: 'Purchase Order ID is required', code: 'REQUIRED' });
        ...
        break;
      case 'adjust':
        if (!operation.product_id) errors.push({ field: 'product_id', message: 'Product ID is required', code: 'REQUIRED' });
        if (!operation.adjustment_type) errors.push({ field: 'adjustment_type', message: 'Adjustment type is required', code: 'REQUIRED' });
        if (operation.quantity_change === 0) errors.push({ field: 'quantity_change', message: 'Quantity change cannot be zero', code: 'INVALID_VALUE' });
        break;
  ...
  private async getFEFOBatches(product_id: string): Promise<FEFOBatch[]> {
    // In real implementation, query database
    // This is a mock implementation
    const mockBatches: FEFOBatch[] = [
      {
  ...
  async receiveStock(request: StockReceiveRequest): Promise<StockOperationResponse> {
    ...
    // Create audit log
    const audit_log = this.createAuditLog(
      'STOCK_RECEIVE',
      'grn_events',
      operation_id,
      request.received_by,
      null,
      { po_id: request.po_id, lines: request.lines }
    );
  ...
  // Stock adjustment
  async adjustStock(request: StockAdjustmentRequest): Promise<StockOperationResponse> {
    ...
    const movement = this.createStockMovement(
      ...
      operation_id,
      'ADJUSTMENT',
      `${request.adjustment_type}: ${request.reason}`,
      undefined,
      request.notes
    );
  ...
  // Return to supplier
  async returnToSupplier(request: ReturnToSupplierRequest): Promise<StockOperationResponse> {
    ...
    for (const line of request.lines) {
      ...
      const movement = this.createStockMovement(
        ...
        operation_id,
        'RETURN',
        `${request.return_type}: ${line.reason}`,
        undefined,
        request.notes
      );
      movements.push(movement);
  ...
  // Customer return
  async customerReturn(request: CustomerReturnRequest): Promise<StockOperationResponse> {
    ...
    for (const line of request.lines) {
      const movement = this.createStockMovement(
        'CUSTOMER_RETURN',
        line.product_id,
        line.quantity, // Positive for return to stock
        'customer_service',
        undefined,
        operation_id,
        'RETURN',
  ...
  // Cycle count
  async cycleCount(request: CycleCountRequest): Promise<StockOperationResponse> {
    ...
    for (const item of request.items) {
      if (item.variance !== 0) {
        if (item.batch_id) batches_affected.push(item.batch_id);
        ...
        const movement = this.createStockMovement(
          ...
          operation_id,
          'ADJUSTMENT',
          `Cycle count variance: System ${item.system_qty}, Counted ${item.counted_qty}`,
          undefined,
          item.notes
        );
        movements.push(movement);
  ...
  async getStockMovements(query: any = {}): Promise<any> {
    ...
    const movements: StockMovement[] = [
      this.createStockMovement(
        'RECEIVE',
        'store_123:paracetamol_500_tab_10s',
        500,
        'user_pharmacy',
        'store_123:PARA500T10:LOT2025A',
        'store_123:PO00087',
        'PO',
        'Goods receipt against PO',
        1.7
...
export async function POST(request: NextRequest) {
  try {
    ...
    let result: StockOperationResponse;

    switch (operation) {
      case 'receive':
        result = await stockService.receiveStock(body as StockReceiveRequest);
        break;
      case 'adjust':
        result = await stockService.adjustStock(body as StockAdjustmentRequest);
        break;
      case 'return-to-supplier':
        result = await stockService.returnToSupplier(body as ReturnToSupplierRequest);
        break;
      case 'customer-return':
        result = await stockService.customerReturn(body as CustomerReturnRequest);
        break;
      case 'cycle-count':
        result = await stockService.cycleCount(body as CycleCountRequest);
        break;
      default:
