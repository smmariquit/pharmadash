...
export async function POST(request: NextRequest) {
  try {
    const body: StockReceiveRequest = await request.json();
    ...
    if (!body.received_by) {
      return NextResponse.json(
        { success: false, message: 'Received by user is required' },
        { status: 400 }
      );
    }
    ...
    for (const [index, line] of body.lines.entries()) {
      ...
      if (!line.lot_no) {
        return NextResponse.json(
          { success: false, message: `Lot number is required for line ${index + 1}` },
          { status: 400 }
        );
      }
      if (!line.expiry) {
        return NextResponse.json(
          { success: false, message: `Expiry date is required for line ${index + 1}` },
          { status: 400 }
        );
      }
      if (line.qty_received <= 0) {
        return NextResponse.json(
          { success: false, message: `Quantity received must be positive for line ${index + 1}` },
          { status: 400 }
        );
      }
      if (line.unit_cost <= 0) {
        return NextResponse.json(
          { success: false, message: `Unit cost must be positive for line ${index + 1}` },
          { status: 400 }
        );
      }
      ...
      if (expiryDate <= new Date()) {
        return NextResponse.json(
          { success: false, message: `Expiry date must be in the future for line ${index + 1}` },
          { status: 400 }
        );
      }
    }
