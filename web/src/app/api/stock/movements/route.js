// web/src/app/api/stock/movements/route.js
...
async function GET(request) {
    try {
        ...
        if (query.limit && (query.limit < 1 || query.limit > 1000)) {
            return server_1.NextResponse.json({ success: false, message: 'Limit must be between 1 and 1000' }, { status: 400 });
        }
        if (query.offset && query.offset < 0) {
            return server_1.NextResponse.json({ success: false, message: 'Offset must be non-negative' }, { status: 400 });
        }
        ...
        if (query.movement_type) {
            ...
            if (!validTypes.includes(query.movement_type)) {
                return server_1.NextResponse.json({ success: false, message: 'Invalid movement type' }, { status: 400 });
            }
        }
        // Validate date range
        if (query.date_from && query.date_to) {
            ...
            if (fromDate > toDate) {
                return server_1.NextResponse.json({ success: false, message: 'date_from cannot be after date_to' }, { status: 400 });
            }
            // Limit date range to prevent performance issues
            const daysDifference = (toDate.getTime() - fromDate.getTime()) / (1000 * 60 * 60 * 24);
            if (daysDifference > 365) {
                return server_1.NextResponse.json({ success: false, message: 'Date range cannot exceed 365 days' }, { status: 400 });
            }
        }
        ...
        let filteredMovements = sampleMovements.filter(movement => {
            if (query.product_id && movement.product_id !== query.product_id)
                return false;
            if (query.batch_id && movement.batch_id !== query.batch_id)
                return false;
            if (query.movement_type && movement.movement_type !== query.movement_type)
                return false;
            if (query.date_from) {
...
async function POST(request) {
    try {
        ...
        if (!body.movements || !Array.isArray(body.movements)) {
            return server_1.NextResponse.json({ success: false, message: 'Movements array is required' }, { status: 400 });
        }
        // In a real implementation, validate and save bulk movements
        // This is typically used for data migration or batch operations
        return server_1.NextResponse.json({ success: false, message: 'Bulk movement creation not implemented in this demo' }, { status: 501 });
    }
    catch (error) {
