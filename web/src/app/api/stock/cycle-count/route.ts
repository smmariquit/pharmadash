...
export async function POST(request: NextRequest) {
  try {
    const body: CycleCountRequest = await request.json();

    // Validation
    if (!body.count_date) {
      return NextResponse.json(
        { success: false, message: 'Count date is required' },
        { status: 400 }
      );
    }
    ...
    if (!body.counted_by) {
      return NextResponse.json(
        { success: false, message: 'Counted by user is required' },
        { status: 400 }
      );
    }
    ...
    if (!body.count_type) {
      return NextResponse.json(
        { success: false, message: 'Count type is required' },
        { status: 400 }
      );
    }
    ...
    if (!validCountTypes.includes(body.count_type)) {
      return NextResponse.json(
        { success: false, message: 'Invalid count type' },
        { status: 400 }
      );
    }
    ...
    if (!body.items || body.items.length === 0) {
      return NextResponse.json(
        { success: false, message: 'At least one item to count is required' },
        { status: 400 }
      );
    }
    ...
    for (const [index, item] of body.items.entries()) {
      if (!item.product_id) {
        return NextResponse.json(
          { success: false, message: `Product ID is required for item ${index + 1}` },
          { status: 400 }
        );
      }
      if (item.system_qty < 0) {
        return NextResponse.json(
          { success: false, message: `System quantity cannot be negative for item ${index + 1}` },
          { status: 400 }
        );
      }
      if (item.counted_qty < 0) {
        return NextResponse.json(
          { success: false, message: `Counted quantity cannot be negative for item ${index + 1}` },
          { status: 400 }
        );
      }
