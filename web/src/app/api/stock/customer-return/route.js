// web/src/app/api/stock/customer-return/route.js
...
async function POST(request) {
    try {
        ...
        if (!body.action) {
            return server_1.NextResponse.json({ success: false, message: 'Return action is required' }, { status: 400 });
        }
        const validActions = ['REFUND', 'EXCHANGE', 'STORE_CREDIT'];
        if (!validActions.includes(body.action)) {
            return server_1.NextResponse.json({ success: false, message: 'Invalid return action' }, { status: 400 });
        }
        ...
        for (const [index, line] of body.lines.entries()) {
            ...
            if (line.unit_price <= 0) {
                return server_1.NextResponse.json({ success: false, message: `Unit price must be positive for line ${index + 1}` }, { status: 400 });
            }
            if (!line.reason) {
        ...
        if (body.action === 'REFUND' && totalRefundAmount > refundThreshold && !body.supervisor_approval) {
            return server_1.NextResponse.json({ success: false, message: 'Supervisor approval required for large refunds' }, { status: 400 });
        }
