...
export async function POST(request: NextRequest) {
  try {
    const body: ReturnToSupplierRequest = await request.json();
    ...
    if (!body.requested_by) {
      return NextResponse.json(
        { success: false, message: 'Requested by user is required' },
        { status: 400 }
      );
    }
    ...
    for (const [index, line] of body.lines.entries()) {
      if (!line.batch_id) {
        return NextResponse.json(
          { success: false, message: `Batch ID is required for line ${index + 1}` },
          { status: 400 }
        );
      }
      if (!line.product_id) {
